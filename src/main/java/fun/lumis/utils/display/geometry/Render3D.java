package fun.lumis.utils.display.geometry;

import com.mojang.blaze3d.platform.GlStateManager;
import com.mojang.blaze3d.systems.RenderSystem;
import fun.lumis.features.impl.render.TargetESP;
...
public class Render3D implements QuickImports {
    ...
    public void drawCube(LivingEntity lastTarget, float anim, float red, String png, int color) {
        ...
        Vector4i finalColor = new Vector4i(color, color, color, color);

        Render3D.drawTexture(entry, Identifier.of(texturePath), -size / 2, -size / 2, size, size, ColorAssist.multRedAndAlpha(finalColor, 1 + red * 10, anim), false);
        matrix.pop();
    }
    private final Random random = new Random();
    private final List<Vec3d> particles = new ArrayList<>();
    public void drawCircle(MatrixStack matrix, LivingEntity lastTarget, float anim, float red, int color) {
        ...
        for (int i = 0; i <= size; i++) {
            ...
            int finalColor = ColorAssist.multRed(color, 1 + red * 125);
            ...
            Render3D.vertexLine(matrix, buffer, start, end,
                    ColorAssist.multAlpha(finalColor, 0.76F * anim),
                    ColorAssist.multAlpha(finalColor, 0F));

            Render3D.drawLine(
                    target.add(cosSin.x, cosSin.y + yAnim, cosSin.z),
                    target.add(nextCosSin.x, nextCosSin.y + yAnim, nextCosSin.z),
                    ColorAssist.multAlpha(finalColor, anim),
                    2, canSee
            );
        }
    ...
    public void drawGhosts(LivingEntity lastTarget, float anim, float red, float speed, int color) {
        Camera camera = mc.getEntityRenderDispatcher().camera;
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0, length = (int) 10.3f; i <= length; i++) {
                ...
                int finalColor = ColorAssist.multRedAndAlpha(color, 1 + red * 10, offset * anim);
                float scale = 0.6f * offset * (0.6f + speed * 0.1f) + bany;

                Render3D.drawTexture(entry, Identifier.of("textures/features/particles/bloom.png"), -scale / 2, -scale / 2, scale, scale, new Vector4i(finalColor, finalColor, finalColor, finalColor), canSee);
            }
        }
    }
