package fun.lumis.utils.interactions.simulate;
import fun.lumis.utils.interactions.simulate.interfaces.Simulation;
...
import fun.lumis.features.impl.player.NoDelay;
...
public class PlayerSimulation implements Simulation, QuickImports {
    ...
    public static PlayerSimulation simulateLocalPlayer(int ticks) {
        PlayerSimulation simulatedPlayer = PlayerSimulation.fromClientPlayer(PlayerSimulation.SimulatedPlayerInput.fromClientPlayer(mc.player.input.playerInput));
    ...
    public void tick() {
        ...
        if (isJumping) {
            ...
            if (inWater && (!onGround || fluidLevel > swimHeight)) {
                ...
                if (player.equals(mc.player) && (!NoDelay.getInstance().isState() || !NoDelay.getInstance().getDelaySetting().isSelected("Прыжок"))) {
                    jumpingCooldown = 10;
                }
            }
        }
    ...
    public static class SimulatedPlayerInput extends Input {
        ...
        public static SimulatedPlayerInput guessInput(PlayerEntity entity) {
            ...
            if (horizontalVelocity > 0.05 * 0.05) {
                double velocityAngle = Simulations.getDegreesRelativeToView(velocity, entity.getYaw());
