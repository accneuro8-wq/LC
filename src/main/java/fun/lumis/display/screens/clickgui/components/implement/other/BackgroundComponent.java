...
public class BackgroundComponent extends AbstractComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String point = " • ";
        DiscordManager discord = lumis.getInstance().getDiscordManager();
        lumis.getInstance().getScissorManager().push(matrix.peek().getPositionMatrix(), 0, 0, window.getScaledWidth(), window.getScaledHeight());
        ...
        lumis.getInstance().getScissorManager().pop();
    }
        ...
        if (MenuScreen.INSTANCE.getCategory() == ModuleCategory.CONFIGS && button == 0) {
            ...
            for (Map<String, Object> map : displayedConfigs) {
                ...
                if (Calculate.isHovered(mouseX, mouseY, configX + 162, configY + 35, 14, 15)) {
                    if (isDefaultTab) {
                        try {
                            File dir = new File(lumis.getInstance().getClientInfoProvider().clientDir(), "Custom");
                            File configFile = new File(dir, config + ".json");
                            String json = new String(Files.readAllBytes(configFile.toPath()));
                            File tempFile = new File(lumis.getInstance().getClientInfoProvider().configsDir(), "temp.json");
                            Files.write(tempFile.toPath(), json.getBytes());
                            lumis.getInstance().getFileController().loadFile("temp.json");
                            tempFile.delete();
    ...
    private List<Map<String, Object>> getCloudConfigs() {
        try {
            if (!lumis.getInstance().getCloudConfigClient().isConnected()) {
            ...
            String response = lumis.getInstance().getCloudConfigClient().sendAndWaitForResponse(message);
    ...
    private void createDefaultConfig() {
        ...
        String name;
        do {
            name = "lumisConfig" + String.format("%03d", random.nextInt(1000));
        } while (new File(dir, name + ".clysm").exists());
        try {
            File configFile = new File(dir, name + ".clysm");
            String json = getCurrentConfigJson();
            Files.write(configFile.toPath(), json.getBytes());
        } catch (IOException e) {
        }
    ...
    private String getCurrentConfigJson() {
        ...
        try {
            lumis.getInstance().getFileController().saveFile("temp.json");
    ...
    private void saveCloudConfig(String name) {
        ...
        data.put("avatar_hash", lumis.getInstance().getDiscordManager().getAvatarId().toString());
    ...
    private void saveCloudConfigWithJson(String name, String json) {
        ...
        try {
            ...
            String message = gson.toJson(request);
            lumis.getInstance().getCloudConfigClient().sendAndWaitForResponse(message);
        } catch (Exception e) {
