...
import fun.lumis.features.module.setting.SettingRepository;
...
public class Module extends SettingRepository implements QuickImports {
    ...
    private void handleStateChange() {
        ...
        if (mc.player != null && mc.world != null) {
            if (state) {
                if (Hud.getInstance().notificationSettings.isSelected("Переключение модулей")) {
    ...
    private void toggleSilent(boolean activate) {
        EventManager eventManager = lumis.getInstance().getEventManager();
        if (activate) {
