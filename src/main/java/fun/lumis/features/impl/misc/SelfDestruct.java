...
import fun.lumis.utils.client.managers.file.impl.PrefixFile;
...
public class SelfDestruct extends Module {
    ...
    public void activate() {
        ...
        lumis.getInstance().getDiscordManager().stopRPC();

        for (Module module : lumis.getInstance().getModuleProvider().getModules()) {
        ...
        lumis.getInstance().getCommandDispatcher().prefix = "" + Calculate.getRandom(0, 9999999);
    ...
    public void onChat(ChatEvent event) {
        ...
        if (msg.equalsIgnoreCase(UserProfile.getInstance().profile("username"))) {
            unhooked = false;
            lumis.getInstance().getDiscordManager().setRunning(true);
            state = false;
            lumis.getInstance().getCommandDispatcher().prefix = ".";
